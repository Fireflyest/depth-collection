...
"""
FLSea Dataset Validation Script for ZoeDepth
...
Usage:
    python flsea_val_zoedepth.py --model-type N --data-root assets/FLSea/red_sea/pier_path --num-samples 10
"""
...
def visualize_comparison_combined(orig_img, proc_img, gt_depth, orig_pred_depth, proc_pred_depth, file_prefix, output_dir):
    ...
    if pred_valid_depths:
        ...
        # 归一化预测深度 (反转，使近处=1红色，远处=0蓝色)
        if np.any(orig_pred_valid):
            orig_pred_viz[orig_pred_valid] = np.clip(1.0 - (orig_pred_depth[orig_pred_valid] - pred_min_depth) / (pred_max_depth - pred_min_depth + 1e-8), 0, 1)
        if np.any(proc_pred_valid):
            proc_pred_viz[proc_pred_valid] = np.clip(1.0 - (proc_pred_depth[proc_pred_valid] - pred_min_depth) / (pred_max_depth - pred_min_depth + 1e-8), 0, 1)
...
def validate_flsea(args):
    """
    Validate ZoeDepth on FLSea dataset
    ...
    # Load ZoeDepth model from local weights
    print(f"Loading ZoeDepth model type: {args.model_type}")
    
    # Import required modules
    from zoedepth.models.builder import build_model
    from zoedepth.utils.config import get_config
    
    # Set local model weights path
    weights_path = os.path.join(args.checkpoint_dir, f"ZoeD_M12_{args.model_type}.pt")
    print(f"Loading model from {weights_path}")
    
    if args.model_type == "N":
        # ZoeD_N - Indoor/outdoor, mono, metric scale, KITTI and NYU Mix
        conf = get_config("zoedepth", "infer")
    elif args.model_type == "K":
        # ZoeD_K - Outdoor, mono, metric scale, KITTI
        conf = get_config("zoedepth", "infer", config_version="kitti")
    elif args.model_type == "NK":
        # ZoeD_NK - Outdoor, mono, metric scale, KITTI with NYU-based encoder
    ...
    # Set use_pretrained_midas to False to avoid torch.hub loading
    conf["use_pretrained_midas"] = False
    conf["train_midas"] = False  # Ensure MiDaS is not trainable
    
    # Build model from config without weights
    model = build_model(conf)
    
    # Apply drop_path compatibility patch
    patch_model(model)
    
    # Manually load weights with more flexible loading
    ...
    for idx in tqdm(indices):
        try:
            ...
            # Convert BGR images to RGB format (ZoeDepth expects RGB)
            ...
            with torch.no_grad():
                ...
                original_tensor = torch.from_numpy(original_rgb).float().permute(2, 0, 1).unsqueeze(0) / 255.0
                processed_tensor = torch.from_numpy(processed_rgb).float().permute(2, 0, 1).unsqueeze(0) / 255.0
                ...
                orig_pred_depth = model.infer(original_tensor).cpu().numpy().squeeze()
                
                # Run model on processed image
                proc_pred_depth = model.infer(processed_tensor).cpu().numpy().squeeze()
...
def monkey_patch_drop_path():
    """
    Monkey patch to fix drop_path compatibility issues in timm/DINOv2 blocks
    """
    try:
        import timm
        # Check if timm blocks need drop_path attribute
        from timm.models.vision_transformer import Block
        if not hasattr(Block, 'drop_path'):
            # Add drop_path as identity function for compatibility
            def identity_drop_path(x):
                return x
            Block.drop_path = identity_drop_path
            print("Applied drop_path compatibility patch for timm.Block")
    except ImportError:
        pass
    
    # Also patch any other potential blocks that might need it
    try:
        # Check if there are any other Block classes that need patching
        import torch.nn as nn
        
        def patch_block_if_needed(module):
            for name, child in module.named_children():
                ...
                patch_block_if_needed(child)
        
        # This will be applied to the model after it's created
        return patch_block_if_needed
    except:
        return lambda x: None

# Apply compatibility patches early
patch_model = monkey_patch_drop_path()

def main():
    parser = argparse.ArgumentParser(description='ZoeDepth Evaluation on FLSea Dataset')
    
    parser.add_argument('--data-root', type=str, default='assets/FLSea/red_sea/pier_path',
                        help='Path to FLSea dataset')
    parser.add_argument('--output-dir', type=str, default='visualizations/flsea_zoedepth',
                        help='Output directory for visualizations and metrics')
    parser.add_argument('--checkpoint-dir', type=str, default='checkpoints',
                        help='Directory containing model checkpoints (ZoeD_M12_N.pt, ZoeD_M12_K.pt, ZoeD_M12_NK.pt)')
    parser.add_argument('--model-type', type=str, default='N',
                        choices=['N', 'K', 'NK'],
                        help='ZoeDepth model type: N (Indoor/outdoor), K (Outdoor), NK (Outdoor with NYU encoder)')
    parser.add_argument('--input-size', type=int, default=518,
                        help='Input size for model inference (Note: ZoeDepth handles resizing internally)')
